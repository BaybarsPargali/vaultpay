...
function PayButton() {
  const { transfer, isTransferring, isReady } = useConfidentialPayment();

  const handlePay = async () => {
    const result = await transfer('recipient_wallet_address', 100);
  ...
  return (
    <button onClick={handlePay} disabled={isTransferring || !isReady}>
      {isTransferring ? 'Processing...' : '🔒 Pay Privately'}
    </button>
  );
}
```
...
│       ├── confidential/route.ts  # CT operations
│       └── batch/route.ts     # Batch payroll
└── hooks/
    ├── useConfidentialPayment.ts  # CT React hook
    ├── useCoSignedPayment.ts      # Co-signer React hook
    └── usePrivatePayment.ts       # Private payment hook
```
